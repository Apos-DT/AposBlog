...
#Vision #速查`,articleSlug:"industrial-vision-system-pitfalls",tagIds:["t-vision","t-cheat"],pinned:!1,createdAt:"2026-04-22T08:00:00.000Z",updatedAt:"2026-04-22T08:00:00.000Z"}],L={tags:j,notes:K};function _(i){if(typeof i!="string")return{meta:{},body:i||""};const c=i.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n?([\s\S]*)$/);if(!c)return{meta:{},body:i};const l={};return c[1].split(/\r?\n/).forEach(s=>{const o=s.match(/^([a-zA-Z0-9_-]+):\s*(.*)$/);if(!o)return;const r=o[1];let t=o[2].trim();if(!t){l[r]="";return}if(/^\[.*\]$/.test(t)){l[r]=t.slice(1,-1).split(",").map(a=>a.trim().replace(/^["']|["']$/g,"")).filter(Boolean);return}if(/^-?\d+(\.\d+)?$/.test(t)){l[r]=Number(t);return}if(t==="true"||t==="false"){l[r]=t==="true";return}l[r]=t.replace(/^["']|["']$/g,"")}),{meta:l,body:c[2]}}function F(i,c){const l=Object.keys(i||{}).filter(o=>i[o]!==""&&i[o]!=null);if(!l.length)return c||"";const s=["---"];return l.forEach(o=>{const r=i[o];Array.isArray(r)?s.push(`${o}: [${r.map(t=>JSON.stringify(t)).join(", ")}]`):typeof r=="string"&&/[:#\[\]]/.test(r)?s.push(`${o}: ${JSON.stringify(r)}`):s.push(`${o}: ${r}`)}),s.push("---",""),s.join(`
`)+(c||"")}const E=/\[\[([^\]|]+?)(?:\|([^\]]+?))?\]\]/g,R=/(^|[\s\(])#([a-zA-Z0-9_\-一-龥]{1,30})\b/g;function N(i){if(!i)return[];const c=new Set;let l;for(E.lastIndex=0;(l=E.exec(i))!==null;)c.add(l[1].trim());return Array.from(c)}function P(i){if(!i)return[];const c=new Set;let l;for(R.lastIndex=0;(l=R.exec(i))!==null;)c.add(l[2]);return Array.from(c)}function y(i){return String(i).replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[c])}function $(i){return y(i)}function G(i,c={}){const{body:l}=_(i||""),s=c.onLink||(()=>null),o=l.replace(/\r\n/g,`
`).split(`
`),r=[];let t=0;for(;t<o.length;){const a=o[t];if(/^```/.test(a)){const d=a.replace(/^```/,"").trim(),g=[];for(t++;t<o.length&&!/^```/.test(o[t]);)g.push(o[t]),t++;t++,r.push(`<pre><code${d?` data-lang="${$(d)}"`:""}>${y(g.join(`
`))}</code></pre>`);continue}if(/^\s*---+\s*$/.test(a)){r.push("<hr/>"),t++;continue}const p=a.match(/^(#{1,6})\s+(.*)$/);if(p){r.push(`<h${p[1].length}>${k(p[2],s)}</h${p[1].length}>`),t++;continue}if(/^>\s?/.test(a)){const d=[];for(;t<o.length&&/^>\s?/.test(o[t]);)d.push(o[t].replace(/^>\s?/,"")),t++;const g=d.join(" ").trim(),S=g.match(/^\[!(INFO|TIP|SUCCESS|NOTE|WARNING|WARN|ERROR|DANGER)\]\s*(.*)$/i);if(S){const w=S[1].toUpperCase(),O={INFO:"info",TIP:"tip",SUCCESS:"success",NOTE:"note",WARNING:"warning",WARN:"warning",ERROR:"error",DANGER:"error"}[w],I={INFO:"Info",TIP:"Tip",SUCCESS:"Success",NOTE:"Note",WARNING:"Warning",WARN:"Warning",ERROR:"Error",DANGER:"Danger"}[w];r.push(`<aside class="md-admon md-admon-${O}"><strong>${I} ·</strong> ${k(S[2],s)}</aside>`)}else r.push(`<blockquote>${k(g,s)}</blockquote>`);continue}if(/^\s*[-*]\s+/.test(a)){const d=[];for(;t<o.length&&/^\s*[-*]\s+/.test(o[t]);)d.push(`<li>${k(o[t].replace(/^\s*[-*]\s+/,""),s)}</li>`),t++;r.push(`<ul>${d.join("")}</ul>`);continue}if(/^\s*\d+\.\s+/.test(a)){const d=[];for(;t<o.length&&/^\s*\d+\.\s+/.test(o[t]);)d.push(`<li>${k(o[t].replace(/^\s*\d+\.\s+/,""),s)}</li>`),t++;r.push(`<ol>${d.join("")}</ol>`);continue}if(/^\s*$/.test(a)){t++;continue}const h=[a];for(t++;t<o.length&&!/^\s*$/.test(o[t])&&!/^(#{1,6}\s|```|>|\s*[-*]\s|\s*\d+\.\s|---+)/.test(o[t]);)h.push(o[t]),t++;r.push(`<p>${k(h.join(" "),s)}</p>`)}return r.join(`
`)}function k(i,c){if(!i)return"";const l=[];let s=i.replace(/`([^`]+?)`/g,(o,r)=>(l.push(r),`\0CODE${l.length-1}\0`));return s=y(s),s=s.replace(/\[\[([^\]|]+?)(?:\|([^\]]+?))?\]\]/g,(o,r,t)=>{const a=r.trim(),p=(t||r).trim(),h=c({type:"wiki",target:a,label:p});return h==null?`<span class="md-wiki md-wiki-missing" title="未链接的笔记: ${$(a)}">${y(p)}</span>`:`<a class="md-wiki" href="${$(h)}" data-wiki="${$(a)}">${y(p)}</a>`}),s=s.replace(/!\[([^\]]*)\]\(([^)\s]+)(?:\s+"[^"]*")?\)/g,(o,r,t)=>{const a=r.trim(),p=t.trim(),h=a.match(/^video(?::(.*))?$/i);if(h){const g=(h[1]||"").trim();return`<figure class="md-media"><video controls preload="metadata" src="${$(p)}"></video>${g?`<figcaption>${y(g)}</figcaption>`:""}</figure>`}const d=a.match(/^audio(?::(.*))?$/i);if(d){const g=(d[1]||"").trim();return`<figure class="md-media"><audio controls preload="metadata" src="${$(p)}"></audio>${g?`<figcaption>${y(g)}</figcaption>`:""}</figure>`}return`<img alt="${$(a)}" src="${$(p)}" loading="lazy"/>`}),s=s.replace(/\[([^\]]+)\]\(([^)\s]+)(?:\s+"[^"]*")?\)/g,(o,r,t)=>{const a=t.trim(),p=/^https?:\/\//i.test(a),d=c({type:"url",target:a,label:r})||a;return`<a href="${$(d)}"${p?' target="_blank" rel="noopener"':""}>${r}</a>`}),s=s.replace(/(^|\s)#([a-zA-Z0-9_\-一-龥]{1,30})\b/g,(o,r,t)=>{const a=c({type:"tag",target:t,label:t});return a?`${r}<a class="md-tag" href="${$(a)}">#${y(t)}</a>`:`${r}<span class="md-tag">#${y(t)}</span>`}),s=s.replace(/\*\*([^*]+?)\*\*/g,"<strong>$1</strong>"),s=s.replace(/(^|[\s>(])\*([^*\n]+?)\*(?=[\s<.,!?:;)]|$)/g,"$1<em>$2</em>"),s=s.replace(/(^|[\s>(])_([^_\n]+?)_(?=[\s<.,!?:;)]|$)/g,"$1<em>$2</em>"),s=s.replace(/ CODE(\d+) /g,(o,r)=>`<code>${y(l[+r])}</code>`),s}function W(){return"n-"+Math.random().toString(36).slice(2,9)+Date.now().toString(36).slice(-4)}const z=C("notes",()=>{const i=Z([]);B("notes",{notes:i},()=>({notes:L.notes}));const c=A(()=>{const n={};return i.value.forEach(e=>{e.title&&(n[e.title.toLowerCase().trim()]=e.id)}),n}),l=A(()=>{const n={};return i.value.forEach(e=>{const u=N(e.content||"");n[e.id]=u.map(f=>c.value[f.toLowerCase().trim()]).filter(f=>f&&f!==e.id)}),n}),s=A(()=>{const n={};return i.value.forEach(e=>{n[e.id]=[]}),Object.entries(l.value).forEach(([e,u])=>{u.forEach(f=>{n[f]&&!n[f].includes(e)&&n[f].push(e)})}),n}),o=A(()=>{const n={};return i.value.forEach(e=>{n[e.id]=P(e.content||"")}),n}),r=A(()=>{const n={};return i.value.forEach(e=>{n[e.id]=_(e.content||"").meta}),n}),t=A(()=>[...i.value].sort((n,e)=>!!n.pinned!=!!e.pinned?n.pinned?-1:1:(e.updatedAt||"").localeCompare(n.updatedAt||"")));function a(n){return i.value.find(e=>e.id===n)}function p(n){if(!n)return null;const e=c.value[n.toLowerCase().trim()];return e?a(e):null}function h(n){return(l.value[n]||[]).map(u=>a(u)).filter(Boolean)}function d(n){return(s.value[n]||[]).map(u=>a(u)).filter(Boolean)}function g(n){const e=a(n);if(!e)return[];const u=N(e.content||""),f=c.value;return u.filter(m=>!f[m.toLowerCase().trim()])}function S(n={}){const e=new Date().toISOString(),u={id:W(),title:n.title||"未命名笔记",content:n.content||"",articleSlug:n.articleSlug||null,tagIds:n.tagIds||[],pinned:!!n.pinned,createdAt:e,updatedAt:e};return i.value.unshift(u),u}function w(n,e){const u=i.value.findIndex(f=>f.id===n);return u<0?null:(i.value[u]={...i.value[u],...e,updatedAt:new Date().toISOString()},i.value[u])}function O(n){i.value=i.value.filter(e=>e.id!==n)}function I(n){const e=a(n);e&&w(n,{pinned:!e.pinned})}function T({keyword:n="",tagId:e=null,articleSlug:u=null,status:f=null}={}){let m=t.value;if(e&&(m=m.filter(v=>(v.tagIds||[]).includes(e))),u&&(m=m.filter(v=>v.articleSlug===u)),f&&(m=m.filter(v=>(r.value[v.id]?.status||"draft")===f)),n){const v=n.toLowerCase();m=m.filter(b=>(b.title||"").toLowerCase().includes(v)||(b.content||"").toLowerCase().includes(v))}return m}const x=A(()=>({total:i.value.length,pinned:i.value.filter(n=>n.pinned).length,withArticle:i.value.filter(n=>n.articleSlug).length,totalLinks:Object.values(l.value).reduce((n,e)=>n+e.length,0),orphans:i.value.filter(n=>{const e=l.value[n.id]?.length||0,u=s.value[n.id]?.length||0;return e+u===0}).length}));return{notes:i,sorted:t,titleIndex:c,outLinksByNote:l,backlinksByNote:s,inlineTagsByNote:o,metaByNote:r,findById:a,findByTitle:p,getOutLinks:h,getBacklinks:d,getMissingLinks:g,create:S,update:w,remove:O,togglePin:I,search:T,stats:x}});export{L as i,_ as p,G as r,F as s,z as u};
